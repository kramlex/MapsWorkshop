...
/**
 * High-level capabilities the chat agent can invoke to interact with the map and the chat UI.
 *
 * This is the surface the agent toolset is built on: each tool ultimately maps to one of these
 * methods. It is intentionally small and side-effecting — searching the map, adding placemarks and
 * driving the chat panel — rather than exposing the underlying [SearchManager] or
 * [PlacemarkRepository] directly.
 *
...
class AssistantApi internal constructor(
    ...
    /**
     * Closes the chat panel, e.g. once the user's request is fully handled or they ask to hide it.
     */
    fun closeChat() {
